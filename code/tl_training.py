...
pickle_name = "../saved_weights/gc_classifier_v1_p27_est.pkl"
...
X_red_train, X_red_test, y_red_train, y_red_test = train_test_split(red_samples,
                                                                    red_labels,
                                                                    test_size=0.05,
                                                                    random_state=42)

X_yellow_train, X_yellow_test, y_yellow_train, y_yellow_test = train_test_split(yellow_samples,
                                                                                yellow_labels,
                                                                                test_size=0.05,
                                                                                random_state=42)

X_green_train, X_green_test, y_green_train, y_green_test = train_test_split(green_samples,
                                                                            green_labels,
                                                                            test_size=0.05,
                                                                            random_state=42)
...
def train_validation_split (dataset_x, dataset_y, train_proportion):
    ...
    for class_size in n_train_classes:
        ...
        # this will be used later to generate augmented_images dataset
    ...
    return X_train_samples, y_train_samples,\
           X_validation_samples, y_validation_samples,\
           train_samples_by_classes, train_labels_by_classes
...
def augment_images (images, generate_count):
    ...
    for i in range(generate_count):
        ...
        pts2 = np.float32([[0 + rand.randint(-a, a) + shiftx,
...
# augmented_images dataset will contain only generated images
augment_limit = 3000
...
def augment_dataset():
    ...
    for augmenting_index in range(n_classes):
        ...
        y_train_augmented = np.concatenate((y_train_augmented,
                                            [augmenting_index for i in range (augment_count)]), axis=0)
